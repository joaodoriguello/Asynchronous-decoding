...
tuple<int, double, double> P2_and_P1_by_P0 (int L, double physical_error, double measu_error, double synchronicity, int time, int precision) {
		...
		for (i = 0; i < L; i++) {
			for (j = 0; j < L; j++) {
				...
				while (m < block[(i+1+L)%L][j].size() and block[(i+1+L)%L][j][m].t0 < t1) {
					...
					p = 0.5*(1 - pow(1 - 2*physical_error, MIN(block[(i+1+L)%L][j][m].t1,t1) - MAX(block[(i+1+L)%L][j][m].t0,t0)));	// Contracted edges' new weight
				...
				while (m < block[i][(j+1+L)%L].size() and block[i][(j+1+L)%L][m].t0 < t1) {
					...
					p = 0.5*(1 - pow(1 - 2*physical_error, MIN(block[i][(j+1+L)%L][m].t1,t1) - MAX(block[i][(j+1+L)%L][m].t0,t0)));	// Contracted edges' new weight
