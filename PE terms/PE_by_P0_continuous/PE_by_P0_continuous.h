...
tuple<int, vector<double>> PE_by_P0_continuous (int L, double physical_error, double measu_error, int time, int precision, int n_shortest_paths) {
	...
	int average = 0;
	vector<double> PE_by_P0(n_shortest_paths);
	for (i = 0; i < n_shortest_paths; i++)
		PE_by_P0[i] = 0.;
...
/////////////////////////////////////////// GET PERFECT MATCHING AND COMPUTE RATIOS
		...
		for (i = 0; i < n_syn; i++) {
			...
			if (i < j) {
				...
				for (l = 0; l < n_shortest_paths; l++)
					PE_by_P0[l] += exp(path_list[0]->dist - path_list[l]->dist);
	...
	return make_tuple(average, PE_by_P0);
}
