...
double CG_decoder_degeneracy_continuous (int L, double physical_error, double measu_error, int time, int precision, double tau) {
		...
		for (i = 0; i < L; i++) {
			for (j = 0; j < L; j++) {
				...
				while (!measu_list.empty()) {
					...
					block[i][j].push_back(block_node({t0,t1,n_blocks}));	// All blocks are numbered by their order of appearance = n_blocks
				...
				block[i][j].push_back(block_node({t0,double(time),n_blocks}));	// All blocks are numbered by their order of appearance = n_blocks
...
/////////////////////////////////////////// GET MATCHING GRAPH AND PERFECT MATCHING
		...
		vector<long double> degeneracy(n_blocks);
		...
		n_syn = syndrome.size();
			...
			degeneracy[syndrome[i].vertex] = 1;
			...
			while (!queue.empty()) { 		// Simplified Dijkstra's algorithm
				...
				for (auto child: block_graph[current]) {	// Get all neighbours of current
					...
					if (distance[child] == -1) {	// Check if child has been visited
						...
						degeneracy[child] = degeneracy[current] * degeneracy_graph[child][current];
						queue.push(child);	// Add child to the queue of vertices to be enquired
					}
					else if (distance[child] == distance[current] + 1)
						degeneracy[child] = MAX(degeneracy[child], degeneracy[current] * degeneracy_graph[child][current]);	// Pick the largest product of time overlaps
				}
			}

	   		for (j = i+1; j < n_syn; j++)
		  		pm->AddEdge(i, j, int(round(1000*(beta*distance[syndrome[j].vertex] - tau*log(degeneracy[syndrome[j].vertex])))));	// Add the distance between anyons i and j
	    	}
...
/////////////////////////////////////////// CORRECT SYNDROME
	    	...
	    	for (i = 0; i < n_syn; i++) {	// Get the anyon matched with i
			j = pm->GetMatch(i);
