...
double AP_decoder_continuous (int L, double physical_error, double measu_error, int time, int precision, double time_weight) {
		...
		n_syn = syndrome.size();
			...
			y0 = syndrome[i].column;
	   		...
	   		for (j = i+1; j < n_syn; j++) {
				...
				dt = fabs((syndrome[i].t1 + syndrome[i].t0) - (syndrome[j].t1 + syndrome[j].t0))/2.;	// Time distance between anyons

				pm->AddEdge(i, j, int(round(1000*(dx + dy + time_weight*dt))));	// Add the distance between anyons i and j
			}
	    	}
