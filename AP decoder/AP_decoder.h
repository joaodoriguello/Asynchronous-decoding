...
double AP_decoder (int L, double physical_error, double measu_error, double synchronicity, int time, int precision, double time_weight) {
...
/////////////////////////////////////////// GET MATCHING GRAPH
		...
		for (i = 0; i < n_syn; i++) {
			...
			for (j = i+1; j < n_syn; j++) {
				...
				dt = abs((syndrome[i].t1 + syndrome[i].t0) - (syndrome[j].t1 + syndrome[j].t0)); // We don't need to divide by 2, instead we multiply the space distance (dx + dy) by 2
		
		   		pm->AddEdge(i, j, int(20*(dx + dy)/time_weight) + int(round(10*synchronicity*dt)) ); // Add the distance between anyons i and j
			}
		}
